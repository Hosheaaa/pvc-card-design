...
class CardDesigner {
    ...
    applyWoodTemplate(card, side) {
        ...
        // 设置竹纹背景图片路径
        const timestamp = new Date().getTime();
        const imagePath = `PVC_templates/Bamboo_${side}.png?t=${timestamp}`;
        backgroundImg.style.backgroundImage = `url('${imagePath}')`;
    ...
    updateEditingRestrictions() {
        // 确定是否应该限制编辑
        // PVC半定制模板（blue/pink）或者金属材质的背面都应该被限制
        // 木质材质不再限制背面编辑
    ...
    addQRCode() {
        ...
        try {
            ...
            const qrElement = {
                ...
                width: 80,  // 默认尺寸
                height: 80,
                src: 'QR_Website.png', // 前端显示时使用文件路径
                rotation: 0,
                opacity: 1
            };
    ...
    async loadQRCodeAsBase64() {
        return new Promise((resolve, reject) => {
            ...
            // 尝试QR码图片路径 - 移动端兼容性改进
            const qrPaths = ['QR_Website.png', './QR_Website.png', '/QR_Website.png'];
            let pathIndex = 0;
    ...
    renderQRCode(element) {
        try {
            ...
            imgElement.onerror = () => {
                ...
                // 尝试多个备用路径
                const fallbackPaths = ['./QR_Website.png', '/QR_Website.png', 'QR_Website.png'];
    ...
    addImageElement(src) {
        try {
            ...
            if (shouldApplyFilter) {
                if (this.currentMaterial === 'wood') {
                    // 木质材质：轮廓雕刻效果，保留边缘，减少填充
                    img.style.filter = 'contrast(2) brightness(0.8) sepia(0.3) saturate(0.2) opacity(0.9)';
                } else if (this.currentMaterial === 'metal') {
                    // 金属材质：单色雕刻效果
    ...
    addTextElement(text, styles = {}) {
        try {
            ...
            if (shouldApplyColorRestriction) {
                if (this.currentMaterial === 'wood') {
                    // 木质材质：雕刻文字效果
                    textElement.style.color = '#5D4037'; // 木质雕刻色
                    textElement.style.textShadow = '1px 1px 2px rgba(0,0,0,0.3)'; // 增加雕刻深度感
                } else if (this.currentMaterial === 'metal') {
                    // 金属材质：金属雕刻色
